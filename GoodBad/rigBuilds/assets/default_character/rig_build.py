...
def load_skinning_data():
    env = environment.Environment()
    root_node = pm.ls('geo', '*_GEO_GRP', '__GEO__')[0]
...
def cleanup():
    ...
    for each in all_root_nodes:
        if str(each) not in ['persp', 'top', 'front', 'side', 'rig']:
            if str(each) in pm.ls('environment', 'geo',  '*_GEO_GRP', '__GEO__'):
                each.setParent('rig')
            else:
                pm.delete(each)
    ...
    geo_transform = pm.ls('geo', '*_GEO_GRP', '__GEO__')[0]
    geo_transform.overrideEnabled.set(1)
    geo_transform.overrideDisplayType.set(2)
